...
int slowmodm2(const puzdef &pd, const setval p1, setval p2) {
   ...
   for (int m1=0; m1<(int)pd.rotgroup.size(); m1++) {
      int m2 = pd.rotinv[m1] ;
      int t = pd.rotinvmap[m2].dat[p1.dat[pd.rotgroup[m2].pos.dat[0]]] - v0 ;
      if (t > 0)
         continue ;
      if (t == 0 && pd.setdefs[0].size > 1) {
         t = pd.rotinvmap[m2].dat[p1.dat[pd.rotgroup[m2].pos.dat[1]]] - v1 ;
         if (t > 0)
            continue ;
      }
      if (t < 0) {
         pd.mul3(pd.rotinvmap[m2], p1, pd.rotgroup[m2].pos, p2) ;
         cnt = 1 ;
         v0 = p2.dat[0] ;
         v1 = p2.dat[1] ;
      } else {
         t = pd.mulcmp3(pd.rotinvmap[m2], p1, pd.rotgroup[m2].pos, p2) ;
         if (t <= 0) {
            if (t < 0) {
               cnt = 1 ;
               v0 = p2.dat[0] ;
