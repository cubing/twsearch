...
void filtermovelist(puzdef &pd, const char *movelist) {
   ...
   for (int i=0; i<(int)moves.size(); i++) {
      moove &mv = moves[i] >= nummoves ?
                pd.expandedrotations[moves[i]-nummoves] : pd.moves[moves[i]] ;
   ...
   for (int i=0; i<(int)pd.basemoves.size() + (int)pd.baserotorders.size(); i++) {
      moove &bm = i >= numbmoves ?  pd.rotations[i-numbmoves] : pd.basemoves[i] ;
      int bmi = i >= numbmoves ? pd.baserotorders[i-numbmoves] : pd.basemoveorders[i] ;
