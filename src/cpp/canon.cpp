...
int recurcanonstates2(const puzdef &pd, int togo, ull moveset, int sp) {
  ...
  for (int i = 0; i < (int)pd.moves.size(); i++) {
    ...
    int nextst = recurcanonstates2(
        pd, togo - 1, (moveset << movebits) + pd.moves[i].cs, sp + 1);
    if (nextst < 0)
  ...
  if (togo == 1) {
    /*
     cout << "Adding values to mask and next; newmask " << hex << newmask << dec
...
void showcanon(const puzdef &pd, int show) {
  ...
  for (int d = 0; d <= canonlim; d++) {
    ...
    if (show) {
      if (d == 0)
        cout << "D " << d << " this " << sum << " total " << gsum << endl
             << flush;
      else
        cout << "D " << d << " this " << sum << " total " << gsum << " br "
             << (sum / osum) << endl
             << flush;
    }
    osum = sum;
