...
impl SemiGroupActionPuzzle for KPuzzle {
    ...
    fn do_moves_commute(&self, move1: &Move, move2: &Move) -> Result<bool, InvalidAlgError> {
        let transformation1 = self.transformation_from_move(move1)?;
        let transformation2 = self.transformation_from_move(move2)?;
        Ok(transformation1.apply_transformation(&transformation2)
