...
/// The `SearchAdaptations` trait bundles various traits that IDFS can invoke to
/// change its search behaviour.
///
/// Each associated type could theoretically be a type parameter on `IDFSearch`
/// itself, but this would make `IDFSearch` types rather unwieldy. So instead of
/// this:
///
...
///
/// In addition, the `DefaultSearchAdaptations` trait can be implemented for any
/// given `SemiGroupActionPuzzle`. This allows us to avoid specifying the
/// `Adaptations` associated type in common. For example, the type for a
/// `KPuzzle` search will often just be:
///
///     IDFSearch<KPuzzle>
///
...
/// instantiated `IDFSearch` without dynamic dispatch.
///
